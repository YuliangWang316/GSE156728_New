...
for (i in c(1:23,25:length(a))) {
  ...
  for (r in 1:length(rownames(Treg_T@assays$RNA@data))) {
    noise<-rnorm(n=length(x=Treg_T@assays$RNA@data[r,]))/1e+05
    Treg_T@assays$RNA@data[r,]<-Treg_T@assays$RNA@data[r,]+noise
  }
  remove(noise,r)
  ...
  for (r in 1:length(rownames(CD8_T@assays$RNA@data))) {
    noise<-rnorm(n=length(x=CD8_T@assays$RNA@data[r,]))/1e+05
    CD8_T@assays$RNA@data[r,]<-CD8_T@assays$RNA@data[r,]+noise
  }
  remove(noise,r)
  ...
  for (j in 1:length(patient)) {
    ...
    assign("c",as.data.frame(t(as.data.frame(b$RNA@data))))
    ...
    assign("n",as.data.frame(t(as.data.frame(m$RNA@data))))
...
write.table(z,"c:/Users/xjmik/Desktop/Rawdata_tumorCD8_tumorTreg.txt",sep = "\t")
...
colnames(scaldata)<-c("a.i.",                   "length.patient." ,      
                      ...
                      "sum.q.IFNG."          ,  "Treg_frequence"       , 
                      "CD8_frequence"        ,  "CD8_ratio" )
write.table(scaldata,"c:/Users/xjmik/Desktop/Rawdata2_tumorCD8_tumorTreg.txt",sep = "\t")